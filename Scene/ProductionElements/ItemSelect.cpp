...
void ItemSelect::Draw()
{
	...
	{
		drawWeaponQualityStorage += (gm.handles.get_quality_element(i) / ((i + 1) * 2)); // 全素材の品質を計算(持ち手部分)
	}
	for (int i = 0; i < gm.main.size(); i++)
	{
		drawWeaponQualityStorage += (gm.main.get_quality_element(i) / ((i + 1) * 2));	// 全素材の品質を計算(上で計算した持ち手部分 + メイン部分)
	}
